> 本文是 [ARCHITECTURE.md](ARCHITECTURE.md) 的中文翻译版本。

# Semantica — 架构

从每种数据源到每种最终输出的完整数据流，以及决策智能（Decision Intelligence）生命周期。

---

## 完整数据管道

每种数据源、每个处理步骤、每个最终产物 —— 汇聚在一张图中。
...
> 注意：`Docs` 和 `Cookbook` 是在本文件之外维护的外部资源，内容可能随时更新。如果链接不可用，请以仓库中的 `README.md` 和仓库内文档作为权威参考。
